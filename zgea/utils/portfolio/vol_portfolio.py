...
class VolPortfolio():
    ...
    def backtest(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        vol = pd.DataFrame(columns=asset_names)
        vol_2 = pd.DataFrame(columns=asset_names)
        self._values = {}
        # get largest volatility value
        max_vol_length = max([v['vol'] for v in self._setup.values()])
        ...
        for name, setup in self._setup.items():
            ...
            # calculate volatility for all given (asset) names
            vol[name] = data[self._setup[name]['vol_asset']].pct_change().iloc[1:].rolling(window=setup['vol']).std() * (int(setup['vol']) ** 0.5)
            # add second volatilty for earlier re-buys
            vol_2[name] = data[self._setup[name]['vol_asset']].pct_change().iloc[1:].rolling(window=setup['vol_2']).std() * (int(setup['vol_2']) ** 0.5)
            # take maximum of last week to avoid buying to early (1 week dead-time)
            vol_2[name] = vol_2.rolling(window=setup['vol_2']).max()
        ...
        portfolio_values = pd.DataFrame(columns=asset_names + ['sum'], index=vol.index[max_vol_length:])
            ...
            for name in asset_names:
                ...
                if (vol.loc[i, name] <= 0.08):
                    if self._values[name] is not None:
                        ...
                        self._log(f"** {i}: [{self._setup[name]['vol_asset']}] Base-Value (${compare_asset_price:.2f}) <= Vol (${vol.loc[i, name]:.2f})")
                        ...
                        # append current date (datetime) to the 'buys' category, since asset is below volatility
                ...
                # use second average (usually defined to be shorter) as additional earlier buying signal
                    ...
                    if self._values[name] is not None:
                        # correct asset price by spread
                        real_asset_price = asset_price * (1 + self._spread)
                        self._log(f"** {i}: [{self._setup[name]['vol_asset']}] Base-Value (${compare_asset_price:.2f}) <= Vol2 (${vol_2.loc[i, name]:.2f})")
                        # get amount of given asset - corrected with costs
                        amount = max((self._values[name] * (1 - self._rel_cost) - self._abs_cost) / real_asset_price, \
                                     (self._values[name] - self._limit) / real_asset_price)
                        self._log(f" => Buy {amount:.2f}x {name} for ${real_asset_price:.2f} each (total: ${self._values[name]:.2f})")
        ...
        for name, _ in self._setup.items():
            n = data[name]
            self._details_memory['chart'][name + '_vol'] = vol[name]
            self._details_memory['chart'][name + '_vol2'] = vol_2[name]
            self._details_memory['chart'][name + '_vol_asset'] = data[self._setup[name]['vol_asset']]
